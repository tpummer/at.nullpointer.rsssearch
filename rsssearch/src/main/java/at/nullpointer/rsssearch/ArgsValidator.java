...
/**
 * Provides different services to validate and extract information from given
 * args
 * 
...
public class ArgsValidator {
    ...
    /**
     * validate the args if they are complete and in a correct order
     * 
     * @param args
     * @return null if args are valid, a String if args are not valid, explaining howto provide valid args
     */
    public static String isValid(String[] args) {
        ...
        if (valid
                && (!Arrays.asList(args).contains(OR) && !Arrays.asList(args)
                        .contains(AND))) {
            valid = valid && false;
        }
        ...
        if (multiTest.contains(OR)) {
            multiTest.remove(OR);
        }
        if (multiTest.contains(AND)) {
            multiTest.remove(AND);
        }
        if (multiTest.contains(FEED)) {
            multiTest.remove(FEED);
        }
        if (valid
                && (multiTest.contains(OR) || multiTest.contains(OR) || multiTest
                        .contains(FEED))) {
            valid = valid && false;
        }
    ...
    /**
     * extract the {@link SearchMode} from the args}
     * 
     * @param args
     * @return {@link SearchMode} if found, if not found null
     */
    ...
    /**
     * extract all the feeds given in the args
     * 
     * @param args
     * @return a {@link List<String>} with the feeds found
     * @throws MalformedURLException
     */
    public static List<String> getFeeds(String[] args)
            throws MalformedURLException {
    ...
    /**
     * extract all the keywords given in the args
     * 
     * @param args
     * @return a {@link String[]} with the keywords found
     */
